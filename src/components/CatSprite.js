...
import { clearMessage, clearThinkMessage } from "../components/redux/spriteSlice";
...
export default function CatSprite() {
  ...
  const { position, rotation, visible, size, message, messageDuration, thinkMessage, thinkMessageDuration } = useSelector((state) => state.sprite);

  const [currentMessage, setCurrentMessage] = useState('');
  const [currentThinkMessage, setCurrentThinkMessage] = useState('');
  ...
  }, [message, messageDuration, dispatch]);

  useEffect(() => {
    if (thinkMessage) {
      setCurrentThinkMessage(thinkMessage);
      const timer = setTimeout(() => {
        setCurrentThinkMessage('');
        dispatch(clearThinkMessage());
      }, thinkMessageDuration * 1000);
      return () => clearTimeout(timer);
    }
  }, [thinkMessage, thinkMessageDuration, dispatch]);
  ...
  return (
    <div style={{position: 'relative', ...catStyle}}>
      <svg
      ...
      {currentMessage && (
        <div className="absolute top-0 left-12 transform -translate-y-full p-1">
          <SpeechBubble message={currentMessage} />
        </div>
      )}
      {currentThinkMessage && (
        <div className="absolute top-0 left-12 transform -translate-y-full p-1">
          <ThinkBubble message={currentThinkMessage}/>
        </div>
      )}
    </div>
